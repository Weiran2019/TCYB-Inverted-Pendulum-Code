...
e4=0.022;
    ...
    lmiterm([1 1 11 X],d1,A');
    lmiterm([1 1 12 X],d12,A');
    lmiterm([1 1 13 X],tao,A');
    lmiterm([1 1 14 X],d13,A');
    ...
    lmiterm([1 7 10 -Y],1,D');
    lmiterm([1 7 11 -Y],d1,B');
    lmiterm([1 7 12 -Y],d12,B');
    lmiterm([1 7 13 -Y],tao,B');
    lmiterm([1 7 14 -Y],d13,B');

    lmiterm([1 8 8 Q7],-1,1);
    lmiterm([1 8 8 Z4],-1,1);

    lmiterm([1 9 9 0],-gama*gama*I1);
    lmiterm([1 9 11 0],d1*Bw');
    lmiterm([1 9 12 0],d12*Bw');
    lmiterm([1 9 13 0],tao*Bw');
    lmiterm([1 9 14 0],d13*Bw');

    lmiterm([1 10 10 0],-I2);

    lmiterm([1 11 11 X],-2*e1,1);
    lmiterm([1 11 11 Z1],e1*e1,1);

    lmiterm([1 12 12 X],-2*e2,1);
    lmiterm([1 12 12 Z2],e2*e2,1);

    lmiterm([1 13 13 X],-2*e3,1);
    lmiterm([1 13 13 Z3],e3*e3,1);

    lmiterm([1 14 14 X],-2*e4,1);
    lmiterm([1 14 14 Z4],e4*e4,1);
   ...
   %% Carry out the simulation with K1
   sim('IP_with_error.slx');
   TCPK1S=CPPAKS.Time;
   TPAK1S=CPPAKS.Time;
   CPK1S=CPPAKS.Data(:,1);
   PAK1S=CPPAKS.Data(:,2);
   save TCPK1S.mat TCPK1S;
   save TPAK1S.mat TPAK1S;
   save CPK1S.mat CPK1S;
   save PAK1S.mat PAK1S;